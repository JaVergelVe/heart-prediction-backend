"""Assemble /users/me payload from ORM rows and apply profile / medical updates."""
...
from app.schemas.user import MedicalConditionsUpdateRequest, UserProfileUpdateRequest


def profile_to_me_dict(profile: UserProfile) -> dict:
...
def medical_to_me_dict(medical: MedicalConditions) -> dict:
    return {
...
def build_me_data(db: Session, user: User) -> dict:
    ...
    return {
        ...
        msg_c.KEY_PROFILE: profile_to_me_dict(profile),
        msg_c.KEY_MEDICAL_CONDITIONS: medical_to_me_dict(medical),
    }


def update_user_profile(
    db: Session, user: User, body: UserProfileUpdateRequest
) -> dict:
    profile = db.scalars(select(UserProfile).where(UserProfile.user_id == user.id)).first()
    if profile is None:
        raise APIError(
    ...
    db.commit()
    db.refresh(profile)
    return profile_to_me_dict(profile)


def update_user_medical_conditions(
    db: Session, user: User, body: MedicalConditionsUpdateRequest
) -> dict:
    medical = db.scalars(
        select(MedicalConditions).where(MedicalConditions.user_id == user.id)
    ).first()
    ...
    db.commit()
    db.refresh(medical)
    return medical_to_me_dict(medical)
